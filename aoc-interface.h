 ...
 *
 *   hash: 61fb1d1efa5c12334b7198d502b783ba6ad06fef
 *
...
inline int AocInterfaceCheck(const void * addr, int size) {
  static const uint8_t uuid_array[UUID_ENTRIES][UUID_SIZE] = { 
  ...
  for (entry = 0; entry < UUID_ENTRIES; entry++) {
    ...
    for (index = start; index < last; index += 1) {
      ...
      ptr = &(((const uint8_t *)addr)[index * UUID_SIZE]);

      for (j = 0; j < UUID_SIZE; j++) {
        if (uuid[j] != ptr[j]) {
          break;
        }
      }
